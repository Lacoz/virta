...
- Support for **multiple workflow definition formats**, including JSONata (inside-step transformations), Amazon States Language (ASL), and Arazzo (OpenAPI-based API workflows).
...
- **BPMN process export** (conceptually aligned with ASL/Arazzo) to allow exchanging processes with BPM tools; exported models should map steps and dependencies into BPMN diagrams without losing DAG semantics.
...
- `import_asl` / `import_arazzo`: register or update Virta pipelines from external specs.
...
4. **Arazzo and BPMN integration scope**
   - Is each format only an import source, or full round-trip (export + import)?
   - How tightly Arazzo should be coupled with OpenAPI spec resolution.
   - How BPMN exports should represent parallelism and error handling while preserving DAG semantics.
